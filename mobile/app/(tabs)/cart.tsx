...
const CartScreen = () => {
  ...
  const handleChackout = () => {
    ...
    if (!addresses || addresses.length === 0) {
      Alert.alert("No Addresses", "Please add a shipping address in your profile before checking out.", [{ text: "OK" }]
      )
      return
    }
  ...
  const handleProceedWithPayment = async (selectedAddress: Address) => {
    ...
    try {
      ...
      // create a payment intent with cart items and shipping address
      ...
      const { error: initError } = await initPaymentSheet({
        ...
        merchantDisplayName: "Shop Cloud"
      })
      ...
      if (presentError) {
        ...
        Alert.alert("Payment cancel", presentError.message)
      } else {
        Sentry.logger.info("Payment successful", {
          total: total.toFixed(2),
          itemCount: cartItems.length
        })
        Alert.alert("Success", "Your payment was successfull! Your order is being processed.", [{ text: "OK", onPress: () => { } }])
        clearCart()
      }
    ...
    <SafeScreen>
      ...
      >
        ...
        <View className='flex-row items-center justify-between mb-4'>
          ...
          <View className='flex-row items-center'>
            <Text className='text-text-primary font-bold text-xl'>RM {total.toFixed(2)}</Text>
          </View>
        </View>
